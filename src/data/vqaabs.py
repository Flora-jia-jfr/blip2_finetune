...
class VQAAbstractDataset(Dataset):
    ...
    def __init__(self, split, mode='q2a', vis_processors=None, text_processors=None, demo=False):
        ...
        if self.demo:
            self.cached_data_dir = os.path.join(data_dir, f'cached_vqaabs_demo_data_for_blip')
        else:
            self.cached_data_dir = os.path.join(data_dir, f'cached_vqaabs_data_for_blip')

        os.makedirs(self.cached_data_dir, exist_ok=True)

        self.cached_data_file = os.path.join(self.cached_data_dir, 'vqaabs_preprocessed_{}.pkl'.format(split))
...
if __name__ == '__main__':
    dataset = VQAAbstractDataset('train')
    # import pdb; pdb.set_trace()
